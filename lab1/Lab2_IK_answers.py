...
def part1_inverse_kinematics(meta_data, joint_positions, joint_orientations, target_pose):
    ...
    for i in range(1, len(path)):
        idx = path[i]
        position_chain[i] = joint_positions[idx]
        if idx in path2:
            # essential
            ...
            orientation_chain[i] = R.from_quat(joint_orientations[idx])
            rotation_chain[i] = R.from_quat(joint_rotations[idx])
            offset_chain[i] = joint_offsets[idx]
        pass
...
def part1_inverse_kinematics2(meta_data, joint_positions, joint_orientations, target_pose):
    """
    完成函数，计算逆运动学
    输入:
        meta_data: 为了方便，将一些固定信息进行了打包，见上面的meta_data类
